...
class HomePlannerVi(
    private val homePlannerContext: Context,
    private val homePlannerCallback: HomePlannerCallBack,
    private val homePlannerWindow: Window
) : WebView(homePlannerContext) {
    private var homePlannerFileChooserHandler: ((ValueCallback<Array<Uri>>?) -> Unit)? = null
    fun homePlannerSetFileChooserHandler(handler: (ValueCallback<Array<Uri>>?) -> Unit) {
        this.homePlannerFileChooserHandler = handler
    }
    init {
        ...
        webSettings.apply {
            ...
            userAgentString = WebSettings.getDefaultUserAgent(homePlannerContext).replace("; wv)", "").replace("Version/4.0 ", "")
        ...
        super.setWebViewClient(object : WebViewClient() {
            override fun shouldOverrideUrlLoading(
                ...
                return if (request?.isRedirect == true) {
                    ...
                    try {
                        homePlannerContext.startActivity(intent)
                    } catch (e: Exception) {
                        Toast.makeText(homePlannerContext, "This application not found", Toast.LENGTH_SHORT).show()
                    }
                    true
                }
            }
            ...
            override fun onPageFinished(view: WebView?, url: String?) {
                ...
                if (url?.contains("https://test-web.syndi-test.net") != true) {
                    ...
                    HomePlannerApplication.homePlannerInputMode = WindowManager.LayoutParams.SOFT_INPUT_ADJUST_RESIZE
                    Log.d(HomePlannerApplication.HOME_PLANNER_MAIN_TAG, "onPageFinished : ${HomePlannerApplication.homePlannerInputMode}")
                    homePlannerWindow.setSoftInputMode(WindowManager.LayoutParams.SOFT_INPUT_ADJUST_RESIZE);
                }
            }
        ...
        super.setWebChromeClient(object : WebChromeClient() {
            ...
            override fun onShowFileChooser(
                ...
                homePlannerFileChooserHandler?.invoke(filePathCallback)
                return true
            }
            override fun onCreateWindow(
                ...
                homePlannerHandleCreateWebWindowRequest(resultMsg)
                return true
            }
        })
    }
    ...
    fun homePlannerFLoad(link: String) {
        super.loadUrl(link)
    }

    private fun homePlannerEnableStableInputScroll() {
        evaluateJavascript(
            """
        (function() {
    ...
    private fun homePlannerHandleCreateWebWindowRequest(resultMsg: Message?) {
        if (resultMsg == null) return
        if (resultMsg.obj != null && resultMsg.obj is WebView.WebViewTransport) {
            val transport = resultMsg.obj as WebView.WebViewTransport
            val windowWebView = HomePlannerVi(homePlannerContext, homePlannerCallback, homePlannerWindow)
            transport.webView = windowWebView
            resultMsg.sendToTarget()
            homePlannerCallback.homePlannerHandleCreateWebWindowRequest(windowWebView)
        }
    }
